...
const RouteLists = ({ date, assignedOrders, setAssignedOrders }: prop) => {
  ...
  const { createTruckRoute, updateTruckRoute, deleteTruckRoute } =
    bindActionCreators(actionCreators, dispatch);

  const truck_routes: TruckRoute[] = useSelector(
  ...
  const fillRouteItems = (route_items_ids: string[], route_types: string[]) => {
    ...
    for (let i = 0; i < route_types.length; i++) {
      ...
      if (route_type === "Order") {
        ...
        route_item = landfills.find(
          (landfill) => landfill.id === route_item_id
        );
  ...
  const analyzeRouteItems = async (truck_route: TruckRoute) => {
    const route_items = fillRouteItems(
      truck_route.route_items,
      truck_route.route_types
    );
    ...
    if (response.status == "OK") {
      const route_data = response.data as Returning_Route_Data[];
  ...
  const deleteRouteItem = async (index: number, route_list_id: string) => {
    const truck_route = truck_routes.find(
      (truck_route) => truck_route.id === route_list_id
    );
    if (truck_route) {
  ...
  const getJSXObject = (
    ...
    if (route_item_type === "Order") {
          ...
          <tr key={new_id}>
            ...
            <td>
              <button
                ...
                onClick={() => deleteRouteItem(index, truck_route_id)}
              >
      ...
      const route_item = landfills.find(
        (landfill) => landfill.id === route_item_id
      );
      if (route_item) {
        return (
  ...
  const insertRouteItems = (truck_route: TruckRoute) => {
    ...
    for (let i = 0; i < route_items.length; i++) {
      const jsx_object = getJSXObject(
        route_items[i],
        route_types[i],
        truck_route.id as string,
        i
      );
  ...
  const insertRouteData = (truck_route: TruckRoute) => {
    ...
    return (
      <div className="row">
        <div className="col l6">Total Duration: {`${hours}:${minute_str}`}</div>
        <div className="col l6">Total Distance: {`${miles} miles`}</div>
      </div>
    );
  };
      ...
      <div key={truck_route.id}>
        <div className="row">
            ...
            <ul className="collapsible">
              <li className="center-align">
                ...
                <div className="collapsible-body">
                  {truck_route.durations.length > 0 ? (
                    insertRouteData(truck_route)
                  ) : (
                    <div></div>
                  )}
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <a
                            ...
                            onClick={() => addRouteItem(truck_route)}
                          >
                  ...
                  <div className="row">
                    <div className="col l4">
                      <a
                        ...
                        onClick={() => analyzeRouteItems(truck_route)}
                      >
                        Analyze Route
                      </a>
                    </div>
                    <div className="col l4">
                      <a
                        ...
                        onClick={() => assignDriver(truck_route)}
                      >
                        Assign Driver
                      </a>
                    </div>
          ...
          <div className="col l1">
            <button
              className="btn black"
              onClick={() => deleteTruckRoute(truck_route)}
            >
  ...
  return (
    <div>
      <button className="btn grey darken-3" onClick={() => addTruckRoute()}>
        Add Route
      </button>
      {insertRouteLists()}
      {addRouteItemActive && truckRoute && (
        <AddRouteItem
          date={date}
          setActive={setAddRouteItemActive}
          assignedOrders={assignedOrders}
          setAssignedOrders={setAssignedOrders}
          truckRoute={truckRoute}
        />
      )}
      {assignDriversActive && truckRoute && (
        <AssignDriver
          setActive={setAssignDriversActive}
          truckRoute={truckRoute}
        />
      )}
    </div>
