...
const CreateLandfillForm = ({ setActive, landfill }: prop) => {
  ...
  useEffect(() => {
    if (landfill) {
      setName(landfill.name);
      setStreet(landfill.street);
      setCity(landfill.city);
      setState(landfill.state);
      setZipcode(landfill.zipcode.toString());
      setLatitude(landfill.latitude.toString());
      setLongitude(landfill.longitude.toString());
    }
  }, [landfill]);
  ...
  const onCreateLandfill = async () => {
    ...
    if (
      ...
      setLoading(true);
      const response = (await createLandfill(
        new_landfill
        ...
        <div className="bg-white px-12 pb-6 rounded-lg shadow-xl z-10 flex flex-col  overflow-hidden ">
          <div className="relative">
            ...
            <div className="flex flex-row justify-center py-6">
              <TrashIcon className="w-12 h-12 text-stone-400" />
            </div>
